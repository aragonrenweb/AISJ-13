...
class wkf_base(models.Model):
    ...
    model_id = fields.Many2one('ir.model', 'Module ID', required=True, help="Select a model that you want to create the Workflow")
    ...
    default_state = fields.Char(compute='_compute_default_state', string="Default Workflow State value", store=False, help='The default Workflow state, It is come from the star node')
    show_states = fields.Char(compute='_compute_default_state', string="Default  States to display", store=False, help='Which status can show the state widget, It is set by node')
    ...
    def make_btm_contain(self):
        ...
        btn_contain.append(XML(btn_wkf_reset_template % {'btn_str': 'Reset Workflow', 'btn_grp': 'base.group_system', 'btn_ctx': self.id, 'no_reset_states': self.no_reset_states }  ))
        return btn_contain
...
class wkf_node(models.Model):
    ...
    wkf_id = fields.Many2one('wkf.base', 'Workflow', required=True, select=True, ondelete='cascade')
    ...
    action = fields.Char('Python Action',size=64, help='When arrived this node, you can set to trigger a object function to do something, example confirm the order')
...
class wkf_trans(models.Model):
    ...
    node_from = fields.Many2one('wkf.node', 'From Node', required=True, select=True, ondelete='cascade',)
    node_to = fields.Many2one('wkf.node', 'TO Node', required=True, select=True, ondelete='cascade')
