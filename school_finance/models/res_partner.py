...
class SchoolFinance(models.Model):
    ...
    family_invoice_ids = fields.Many2many("account.move", compute="_compute_family_invoice_ids", store=True,
                                          domain=[('type', '=', 'out_invoice')],
                                          context={'default_type': 'out_invoice', 'type': 'out_invoice',
    ...
    invoice_amount_tax_signed = fields.Monetary(string="Total Tax",
                                                compute="_compute_invoice_totals")
    invoice_amount_total_signed = fields.Monetary(string="Total Total",
                                                  compute="_compute_invoice_totals")
    invoice_amount_residual_signed = fields.Monetary(string="Total Amount Due",
                                                     compute="_compute_invoice_totals")
    student_invoice_amount_untaxed_signed = fields.Monetary(string="Student Total Tax Ecluded",
                                                            compute="_compute_invoice_totals")
    student_invoice_amount_tax_signed = fields.Monetary(string="Student Total Tax",
                                                        compute="_compute_invoice_totals")
    ...
    family_invoice_amount_untaxed_signed = fields.Monetary(string="Family Total Tax Ecluded",
                                                           compute="_compute_invoice_totals")
    family_invoice_amount_tax_signed = fields.Monetary(string="Family Total Tax",
                                                       compute="_compute_invoice_totals")
    family_invoice_amount_total_signed = fields.Monetary(string="Family Total Total",
                                                         compute="_compute_invoice_totals")
...
class FinacialResponsabilityPercent(models.Model):
    ...
    category_id = fields.Many2one("product.category", required=True, string="Category",
    ...
    def _get_family_domain(self):
        ...
        return {
            'domain': {'family_id': [("is_family", "=", True), ("is_company", "=", True), ('id', 'in', family_ids)]}}
